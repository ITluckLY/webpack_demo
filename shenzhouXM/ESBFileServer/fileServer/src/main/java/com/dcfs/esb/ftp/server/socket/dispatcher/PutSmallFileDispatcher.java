 ...
 */
public class PutSmallFileDispatcher {
    private static final Logger log = LoggerFactory.getLogger(PutSmallFileDispatcher.class);
    ...
    public PutSmallFileDispatcher(CachedContext context, FileMsgBean bean, FtpConnector conn) {
        this.context = context;
        this.bean = bean;
        this.conn = conn;
        cxtBean = context.getCxtBean();
        nano = cxtBean.getNano();
        cxtBean.setPreProcessReturn(false);
        cxtBean.setProcessReturn(false);
    }

    public void run() {
        CapabilityDebugHelper.markCurrTime("PutSmallFileDispatcherBegin");
        try {
            conn.setTcpNoDelay(true);
            preprocess();
            //下载文件时，认证通过并且文件存在，才能进行文件下载
            if (FileMsgTypeHelper.isGetFileAuth(bean.getFileMsgFlag())
                    && BooleanTool.anyFalseByNull(bean.isAuthFlag(), bean.isFileExists())) {
                context.setCanntInvokeNextFlow(true);
            }
            //前面没有消费文件数据则校验不通过也要消费掉文件数据
            ...
            CapabilityDebugHelper.markCurrTime("readAckBegin");
            //接收到确认消息再关闭socket,fix RST
            conn.readAck();
            CapabilityDebugHelper.markCurrTime("readAckEnd");
        } catch (FtpException e) {
            FLowHelper.setError(context, e.getCode());
            bean.setErrCode(e.getCode());
            bean.setFileRetMsg(FLowHelper.getErrorMsg(context));
            log.error("nano:{}#", nano, e);
        } finally {
            postprocess();
            CapabilityDebugHelper.markCurrTime("PutSmallFileDispatcher-colseConnBegin");
            //回收资源
            closeConn();
            CapabilityDebugHelper.markCurrTime("PutSmallFileDispatcher-colseConnEnd");
            //回收令牌资源
            ...
            CapabilityDebugHelper.markCurrTime("PutSmallFileDispatcherEnd");
        }
    }
    ...
    protected void preprocess() {
        ...
        String[] preServices = PutSmallFileDispatcher.PRE_SERVICES;
        ServiceContainer serviceContainer = ServiceContainer.getInstance();
        CapabilityDebugHelper.markCurrTime("PutSmallFileDispatcher-preServices");
        for (String service : preServices) {
            if (context.isCanntInvokeNextFlow()) break;
            log.debug("nano:{}#Dispatcher正在执行前置基础服务{}", nano, service);
            if (CapabilityDebugHelper.isDebug()) CapabilityDebugHelper.markCurrTime("PutSmallFileDispatcher-getService0--" + service);
            try {
            ...
            if (CapabilityDebugHelper.isDebug()) CapabilityDebugHelper.markCurrTime("PutSmallFileDispatcher-getService1--" + service);
        }
        //设置加密
        //if (bean.isAuthFlag() && bean.isScrtFlag()) bean.setDesKey(ScrtUtil.getDesKey()); //NOSONAR
        //设置分片大小
        //bean.setPieceNum(NetworkSpeedCtrlHelper.getPieceNum(context));//NOSONAR
        if (FLowHelper.hasError(context)) {
    ...
    protected void process() {
        ...
        ServiceContainer serviceContainer = ServiceContainer.getInstance();
        for (String service : serviceFlow.getBaseService()) {
            if (context.isCanntInvokeNextFlow()) break;
