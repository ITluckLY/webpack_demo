...
public class ComponentService extends AbstractService {
  ...
  public ResultDto<List<ComponentModel.Service>> getlistAll(String name) {
    ComponentModel model = getloadModel(name);
    return ResultDtoTool.buildSucceed(model.getServices());
  }
  ...
  private ComponentModel getloadModel(String fileName) {
    if (fileName.equals( "FilesProcesFlow")) {
      return cfgFileService.loadModel4Name(FILE_PROCESS_NAME, ComponentModel.class);
    } else {
      return cfgFileService.loadModel4Name(CFG_FILE_NAME, ComponentModel.class);
    }
  }
