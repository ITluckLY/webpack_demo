...
public class CfgModelConverter {
  ...
  // netty
  public static void convertTo(FtUserNetty ftUserNetty, NettyModel.NettyConfig nettyConfigFile) {
    NettyModel.Prarm prarm = new NettyModel.Prarm();
    prarm.setMaxSpeed(ftUserNetty.getMaxSpeed());
    prarm.setSleepTime(ftUserNetty.getSleepTime());
    prarm.setScanTime(ftUserNetty.getScanTime());
    List<NettyModel.Prarm> prarmList =nettyConfigFile.getPrarm();
    prarmList.add(prarm);
    nettyConfigFile.setPrarm(prarmList);

  }
  // 此处设置 nettyConfig 里面的值 ，里面没有属性，先不设置
  // newNetty
  public static void convertToNetty(FtUserNetty userAuthModel, NettyModel.NettyConfig nettyConfigFile) {

  }

  // twonettyService
  public static void convertToNettyService(FtUserNetty ftUserNetty, TwoNettyModel.NettyConfig nettyConfigFile) {
    //
    TwoNettyModel.Prarms prarms = new TwoNettyModel.Prarms();
    prarms.setMaxsPeed(ftUserNetty.getMaxSpeed());
    prarms.setSleepTime(ftUserNetty.getSleepTime());
    prarms.setScanTime(ftUserNetty.getScanTime());
    List<TwoNettyModel.Prarms> pram = nettyConfigFile.getPrarm().getPrarms();
    pram.add(prarms);
    nettyConfigFile.setPrarm((TwoNettyModel.Prarm) pram);

    TwoNettyModel.ChannelSpeed channelSpeed = nettyConfigFile.getChannelSpeed();

    TwoNettyModel.NettyChannel channe = new TwoNettyModel.NettyChannel();
    channe.setUserName(ftUserNetty.getUserName());
    channe.setReadLimit(ftUserNetty.getReadLimit());
    channe.setWriteLimit(ftUserNetty.getWriteLimit());

    List<TwoNettyModel.NettyChannel> channelList2 = nettyConfigFile.getChannelSpeed().getChannelList();
    channelList2.add(channe);
    channelSpeed.setChannelList(channelList2);
    nettyConfigFile.setChannelSpeed(channelSpeed);
  }

  public static void convertToFtUserNetty(NettyModel.NettyConfig userAuthModel, FtUserNetty nettyConfigFile) {
  /*  nettyConfigFile.setMaxsPeed(userAuthModel.getPrarm().getMaxsPeed());
    nettyConfigFile.setSleepTime(userAuthModel.getPrarm().getSleepTime());
    nettyConfigFile.setScanTime(userAuthModel.getPrarm().getScanTime());*/

    nettyConfigFile.setUserName(userAuthModel.getChannelSpeed().getChannelList().get(0).getUserName());
    nettyConfigFile.setReadLimit(userAuthModel.getChannelSpeed().getChannelList().get(0).getReadLimit());
    nettyConfigFile.setWriteLimit(userAuthModel.getChannelSpeed().getChannelList().get(0).getWriteLimit());

  }

  // twoNetty
  public static void convertToTwoNetty(TwoNettyModel.NettyConfig nettyConfigFile, FtUserNetty ftUserNetty) {
    TwoNettyModel.Prarm prarm = nettyConfigFile.getPrarm();
    //ftUserNetty.setMaxsPeed();
    TwoNettyModel.ChannelSpeed channel = nettyConfigFile.getChannelSpeed();
  ...
  // addPrarm 使用
  public static void convertToP(FtUserNetty ftUserNetty , List<NettyModel.Prarm>  nettyPrarm ) {
   /* NettyModel.Prarm nmp = new NettyModel.Prarm();
    nmp.setMaxsPeed(ftUserNetty.getMaxsPeed());
    nmp.setSleepTime(ftUserNetty.getSleepTime());
    nmp.setScanTime(ftUserNetty.getScanTime());

    List<NettyModel.Prarm> prarms= new ArrayList<>();
    prarms.add(nmp);
    nettyPrarm.setPrarm(prarms);*/
    NettyModel.Prarm nmp = new NettyModel.Prarm();
    nmp.setMaxSpeed(ftUserNetty.getMaxSpeed());
    nmp.setSleepTime(ftUserNetty.getSleepTime());
    nmp.setScanTime(ftUserNetty.getScanTime());
  ...
  // addPrarm2 使用
  public static void convertToP2(FtUserNetty ftUserNetty , NettyModel.Prarm  nettyPrarm ) {
    NettyModel.Prarm nmp = new NettyModel.Prarm();
    nettyPrarm.setMaxSpeed(ftUserNetty.getMaxSpeed());
    nettyPrarm.setSleepTime(ftUserNetty.getSleepTime());
    nettyPrarm.setScanTime(ftUserNetty.getScanTime());

  }


  // 添加 ChannelSpeed addChannel 使用
  public static void convertToC(FtUserNetty ftUserNetty, NettyModel.NettyConfig channel) {
    NettyModel.NettyChannel  nc = new NettyModel.NettyChannel();
    nc.setUserName(ftUserNetty.getUserName());
    nc.setWriteLimit(ftUserNetty.getWriteLimit());
    nc.setReadLimit(ftUserNetty.getReadLimit());

    List<NettyModel.NettyChannel> channelList =new ArrayList<>();
    channelList.add(nc);

//    NettyModel.ChannelSpeed  ncs = channel.getChannelSpeed();
    NettyModel.ChannelSpeed  ncs = new NettyModel.ChannelSpeed();
    ncs.setChannelList(channelList);
    channel.setChannelSpeed(ncs);
  }



  // 添加 ChannelSpeed
  public static void convertToC2(FtUserNetty ftUserNetty, NettyModel.ChannelSpeed  channel) {
    NettyModel.NettyChannel  nc = new NettyModel.NettyChannel();
    nc.setUserName(ftUserNetty.getUserName());
    nc.setWriteLimit(ftUserNetty.getWriteLimit());
    nc.setReadLimit(ftUserNetty.getReadLimit());
  ...
  // 添加 updateP
  public static void convertToUP(NettyModel.NettyConfig nettyConfig,FtUserNetty ftUserNetty) {

    NettyModel.Prarm np =nettyConfig.getPrarm().get(0);
    ftUserNetty.setMaxSpeed(np.getMaxSpeed());
    ftUserNetty.setSleepTime(np.getSleepTime());
    ftUserNetty.setScanTime(np.getScanTime());

  }

  public static void convertToNettyP(FtUserNetty ftUserNetty, NettyModel.NettyConfig nettyConfigFile) {
    NettyModel.Prarm prarm = new NettyModel.Prarm();
    prarm.setMaxSpeed(ftUserNetty.getMaxSpeed());
    prarm.setSleepTime(ftUserNetty.getSleepTime());
    prarm.setScanTime(ftUserNetty.getScanTime());
  ...
  public static void convertToNettyC(FtUserNetty ftUserNetty, NettyModel.NettyConfig nettyConfigFile) {
    NettyModel.NettyChannel prarm = new NettyModel.NettyChannel();
    prarm.setUserName(ftUserNetty.getUserName());
    prarm.setReadLimit(ftUserNetty.getReadLimit());
    prarm.setWriteLimit(ftUserNetty.getWriteLimit());
    List<NettyModel.NettyChannel> prarmList =nettyConfigFile.getChannelSpeed().getChannelList();
    //prarmList.remove(0);
    prarmList.add(prarm);
    NettyModel.ChannelSpeed cs =nettyConfigFile.getChannelSpeed();
    cs.setChannelList(prarmList);
    nettyConfigFile.setChannelSpeed(cs);
