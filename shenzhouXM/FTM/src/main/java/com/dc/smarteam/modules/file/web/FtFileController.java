...
public class FtFileController extends BaseController {
    ...
    public String index(FtServiceNode ftServiceNode, User user, Model model, HttpServletRequest request, HttpServletResponse response, RedirectAttributes redirectAttributes) {
        if (null != ftServiceNode.getState() && ftServiceNode.getState().trim().equalsIgnoreCase("1")) {
            ...
            model.addAttribute("nodeName", ftServiceNode.getName());
        } else {
            addMessage(redirectAttributes, "节点未连接，请检查连接状态后，重新再试！");
            return "redirect:" + Global.getAdminPath() + "/servicenode/ftServiceNode";//NOSONAR
        }
        return "modules/file/ftFileIndex";
    }
