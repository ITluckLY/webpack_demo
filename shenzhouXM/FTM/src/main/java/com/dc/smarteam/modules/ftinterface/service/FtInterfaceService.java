...
import com.dc.smarteam.tool.ResultDtoTool;
...
public class FtInterfaceService {
    ...
    public ResultDto<String> user(FtInterface ftInterface) throws IOException {
        ...
        if (null == operateType) {
            log.error("操作类型为空");
            resul = ResultDtoTool.buildError("操作类型不能为空");
        }
...
//        ResultDto<String> resultDto = null;
    ...
    public ResultDto<String> serviceInfo(FtInterface ftInterface) throws IOException{
        ...
        if (resultDto.getCode().equals("0000")) {
            ...
            List<FtFile> file = ftGetFileService.findFileList(ftFile);

            rep = ResultDtoTool.buildSucceed(file);

        }else rep =ResultDtoTool.buildError("用户无此交易码权限");
