        ...
        $.ajax({
            ...
            success: function (data) {
                $.each(data, function (i, item) {
                    // 不显示nameNode实时监控
                    if (item.system != null) {
                        $("#container").append("<div id=\'memory" + i + "\' style='height:400px; width: 95%;'></div>");
                        $('#memory' + i + '').highcharts('StockChart', {
                            chart: {
                                renderTo: "container",
                                events: {
                                    load: function () {
                                        ...
                                        setInterval(function () {
                                            $.ajax({
                                                ...
                                                url: '${ctx}/monitor/FtNodeMonitor/RealTime/memory?nodename=' + item.node,
                                                type: "get",
                                                dataType: 'json',
                                                success: function (data) {
...
//                                                    var y = currentTime - updateTime > 12 * period ? null : parseInt(data.memory);
                                                    var x = dataTime = (new Date(data.time)).getTime();
                                                    var y = parseFloat(data.memory) < 0 ? null : parseFloat(data.memory);
