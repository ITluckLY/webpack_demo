...
<!-- 	</ul> -->
	<form:form id="searchForm" modelAttribute="log" action="${ctx}/sys/log/" method="post" class="breadcrumb form-search">
		...
		<div>
			<label>操作菜单：</label><input id="title" path="title" type="text" maxlength="50" class="input-mini" value="${log.title}"/>
	...
	<table id="contentTable" class="table table-striped table-bordered table-condensed" style="width: 99%">
		...
		<c:forEach items="${page.list}" var="logTemp">
			<tr>
				<td>${logTemp.title}</td>
				<td>${logTemp.createBy.name}</td>
				<td>${logTemp.createBy.company.name}</td>
				<td>${logTemp.createBy.office.name}</td>
				<td><strong>${logTemp.requestUri}</strong></td>
				<td>${logTemp.method}</td>
				<td>${logTemp.remoteAddr}</td>
				<td><fmt:formatDate value="${logTemp.createDate}" type="both"/></td>
			</tr>
			<c:if test="${not empty logTemp.exception}"><tr>
				<td colspan="8" style="word-wrap:break-word;word-break:break-all;">
<%-- 					用户代理: ${log.userAgent}<br/> --%>
<%-- 					提交参数: ${fns:escapeHtml(log.params)} <br/> --%>
					异常信息: <br/>
					${fn:replace(fn:replace(fns:escapeHtml(logTemp.exception), strEnter, '<br/>'), strTab, '&nbsp; &nbsp; ')}</td>
			</tr></c:if>
		</c:forEach>
		</tbody>
	</table>
