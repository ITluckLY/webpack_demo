...
<script type="text/javascript">
    d3.json("${ctx}/monitor/FtNodeMonitor/allnodeInfo", function (error, data) {
        ...
        data.forEach(function (d) {
            nodes.push({id: d.node, type: 'switch', state: parseInt(d.state || '0'), system: d.system, expand: false});
        ...
        function Topology(ele) {
            ...
            var w = ele.clientWidth,
                h = ele.clientHeight,
                self = this;
            this.force = d3.layout.force().gravity(.1).distance(100).charge(-1500).linkStrength(0.7).size([w, h]);
...
//                        console.debug(d);
                    ...
                    if (timeStampNow - timeStampInit > 2000) {
                        nodetrasfor(d.id, d.system);
                        nodeinfo(d.id);
                        d.expand && self.clickFn(d);
                        timeStampInit = timeStampNow;
                    } else {
                        alert("3秒内请勿多次点击...")
                    }
                });
